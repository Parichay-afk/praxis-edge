...
```bash
npm run build:og   # rewrites public/og-image.png from scripts/build-og.mjs
```
...
- [x] OG image: generated by `scripts/build-og.mjs` → `public/og-image.png`.
- [x] Self-host portfolio images: done. Images live in [src/assets/work/](src/assets/work) and are optimized by Astro's `<Image>` component into responsive WebP variants.
- [ ] Delete `praxis_edge.html` once the Astro build is verified in production.
- [ ] Per-service / per-case-study pages.
- [ ] Insights / MDX blog.
- [ ] EN ↔ AR with RTL.
